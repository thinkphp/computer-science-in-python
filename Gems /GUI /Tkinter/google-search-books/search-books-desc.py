    ...
    def display_results(self, items):
        ...
        for item in items:
            ...
            if thumbnail_url:
                ...
                # Create a label with the image and bind it to open the info link
                thumbnail_label = tk.Label(book_frame, image=photo, cursor="hand2")
                thumbnail_label.image = photo  # Keep a reference
                thumbnail_label.pack(side="left")
                thumbnail_label.bind("<Button-1>", lambda e, link=info_link: self.open_link(link))
            ...
            description_button = tk.Button(details_frame, text="Show Description", font=("Arial", 12), fg="blue", cursor="hand2", command=lambda desc=description: self.show_description(desc))
            description_button.pack(anchor="w", pady=5)

    def open_link(self, link):
        webbrowser.open(link)

    def show_description(self, description):
        # Create a new top-level window for the description
        description_window = tk.Toplevel(self.root)
        description_window.title("Book Description")
        description_window.geometry("400x300")

        # Add a text widget to display the description
        text_widget = tk.Text(description_window, wrap="word", font=("Arial", 12))
